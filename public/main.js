...
const handleGetClima = () => {
  ...
  axios.get(`http://localhost:4000/coordenadas?direccion=${encodeURIComponent(direccion)}`)
...
const getClima = async (long, lat) => {
  try {
    const { data } = await axios.get(`http://localhost:4000/clima?lat=${encodeURIComponent(lat)}&long=${encodeURIComponent(long)}`);
